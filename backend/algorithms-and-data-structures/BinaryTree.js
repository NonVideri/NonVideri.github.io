class BinaryTree {
  ...
  insert(value) {
    if (value < this.value) {
      if (!this.left) this.left = new BinaryTree(value, this.depth + 1);
      else this.left.insert(value);
    } else {
      if (!this.right) this.right = new BinaryTree(value, this.depth + 1);
  ...
  getNodeByValue(value) {
    ...
    if (this.right) return this.right.getNodeByValue(value);
    return null;
  }
